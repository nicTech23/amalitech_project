...
const FeedProvider = ({children}) => {
  ...
  const [not_found, set_not_found] = useState(false)
    ...
    const get_all_feeds = async()=>{
       try {
         const response = await axios.get("https://amalitech-project-server.onrender.com/api/v1/document-route/get-all-files")
  ...
  const post_message = async()=>{
    ...
    try {
      ...
      const response = await axios.post(`https://amalitech-project-server.onrender.com/api/v1/message-route/send-message/${feed_id}/`, body, {
  ...
  const post_download = async(feed_id, file)=>{
    try {
       const user_token = Cookies.get("user_token")
      const response = await axios.get(`https://amalitech-project-server.onrender.com/api/v1/download-route/download-file/${feed_id}/`,  {
  ...
  const search_feed = async ()=>{
    try {
      if (search !== "") {
        const user_token = Cookies.get("user_token")
        const response = await axios.get(`https://amalitech-project-server.onrender.com/api/v1/document-route/search-file?search=${search}`, {
          withCredentials: true,
          headers: {
            'Authorization': `Bearer ${user_token}`,
          }
